...
void IPCShowMessageInput::initButtons()
{
	...
	this->buttons["SHOWMESSAGE"] = new Button(630, 250, 244, 44, &this->font, "Show message");
	this->buttons["SEND"] = new Button(630, 350, 244, 44, &this->font, "Send");
...
void IPCShowMessageInput::update(const float& dt)
{
	...
	if (input.keycounter < 1) {
		...
		try {
			PCB::showMessages(PCB::getPCB(System::CPU.getRunningPID()));
		}
